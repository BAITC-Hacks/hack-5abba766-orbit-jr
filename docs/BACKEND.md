...
Сначала строится воспроизводимый baseline. Предлагаемый порядок сравнения кандидатов v1: число полностью закрытых критических разрывов; затем `weighted_direct_gain + 0.5 × weighted_unlocked_gain`; затем меньше негативных исходов в последних трёх завершённых/отклонённых/прерванных/пропущенных участиях того же события; затем меньшие трудозатраты; tie-break по event ID. Вес критического навыка 2, остальных 1. Для нескольких открываемых событий берём максимальную будущую пользу, не суммируем взаимоисключающие обещания. Это продуктовая эвристика для проверки, не обученная метрика и не доказанная вероятность успеха.
