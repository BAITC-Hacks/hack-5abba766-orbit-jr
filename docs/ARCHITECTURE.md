...
В `backend/` реализованы хранение, миграции, bootstrap, доменные расчёты, HTTP, сессии, импорт, симуляции, учебные попытки с серверной проверкой и адаптер AI. Это исполняемый код, а не только спецификация. Без настроенного ключа сервер явно возвращает `rules_fallback`. Живые командные проверки на авторских примерах зафиксированы в [AI_VERIFICATION.md](AI_VERIFICATION.md); они не подтверждают настройки текущего локального запуска. Детали запуска и общие проверки приведены в корневом [README](../README.md).

## Документы и исходники

- [Бэкенд подробно](BACKEND.md): данные, расчёты, API, транзакции и ограничения.
- [Пользовательская механика](PRODUCT_GUIDE.md): карьерный путь, объяснения, обучение и чтение HR-сводки.
- [Общие TypeScript-контракты](../contracts/backend.ts): формат запросов, ответов и входа AI.
- [Контракт обучения](../contracts/learning.ts): публичные модули, попытки и результаты проверки.
...
flowchart TD
    ...
    HTTP --> LEARN[learning/service: уроки и проверка]
    LEARN --> DATA
    LEARN --> DB
...
Обучение также не обращается к модели. Два авторских модуля содержат по три урока и три вопроса. Сервер сохраняет порядок пройденных уроков и проверяет ответы по снимку материала конкретной попытки. При успехе `learning/service` вызывает общую операцию симуляции внутри той же транзакции; изменение навыков, учебный результат и квитанции повторов фиксируются вместе.
...
contracts/learning.ts                публичный контракт обучения
...
backend/src/learning/                авторский контент, приватные ответы, попытки
...
Session-level блокировки подборов используют отдельный `recommendationLockPool` с пределом 10 соединений, обычные чтения и записи — другой пул с пределом 10. Ожидание AI-пула ограничено секундой. Внутри процесса допускается не более трёх подборов, успешные AI-ответы кэшируются на 60 секунд. Одинаковые вызовы без сигнала отмены объединяются; HTTP-запросы независимы и передают `AbortSignal` до транспорта, чтобы отмена одного не прервала чужую работу. Подробная семантика и ошибки — в [BACKEND.md](BACKEND.md).
...
Compose запускает PostgreSQL и приложение. Перед HTTP-сервером bootstrap применяет миграции и один раз загружает датасет. `001_initial` создаёт основное хранилище, `002_learning` добавляет учебные попытки без пересоздания исходных таблиц. Файлы источника подключаются только для чтения, в Git и образ они не включаются. Уже инициализированная БД сохраняет цели, импорт, симуляции, обучение и сессии; повторный bootstrap не заменяет их исходными файлами.
...
Успешный мини-тест подтверждает прохождение авторского демомодуля в приложении. После него симулируется эффект полной активности из каталога; семиминутный материал не заменяет исходный 16/24-часовой курс. Приватные ключи ответов хранятся на сервере, а клиент получает только уроки и варианты ответа; пояснения возвращаются после проверки.
