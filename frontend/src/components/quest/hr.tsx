...
export function Hr({ onError }: { onError: (error: unknown) => void }) {
  ...
  const selectedProfile = useRef<HTMLElement | null>(null);
  useEffect(() => {
    if (selected) selectedProfile.current?.scrollIntoView({ block: "start" });
  }, [selected]);
  ...
  return (
    <>
      ...
      {selected && (
        <section className="selected-employee" ref={selectedProfile}>
