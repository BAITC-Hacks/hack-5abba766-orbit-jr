  ...
  return (
    <section className="skills-panel">
      ...
      <div className="skill-list">
        ...
        {employee.goal.target && gaps.length === 0 && <p className="cq-skills-covered">Все требования выбранной цели закрыты.</p>}
        {covered.length > 0 && <div className="cq-skill-group cq-covered-skills"><h3>Уже соответствует цели <span>{covered.length}</span></h3>{rows(covered)}</div>}
        ...
        {other.length > 0 && <details className="cq-other-skills"><summary>{employee.goal.target ? "Остальные навыки" : "Все навыки профиля"}<span>{other.length}</span></summary>{rows(other)}</details>}
      </div>
    </section>
  );
}
