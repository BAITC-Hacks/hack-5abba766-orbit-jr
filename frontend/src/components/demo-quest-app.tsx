...
export default function QuestApp({
    ...
    if (ready) {
      try {
        localStorage.setItem(
          "cq-ui-v1",
          JSON.stringify({ done, started, goal }),
        );
      } catch {
  ...
  return (
    <>
      ...
      <dialog
        aria-label="Демонстрационное окно"
        ref={modalRef}
        onCancel={close}
        onClick={(e) => {
            ...
            <ul className="fact-list">
              {[
                ...
                `Формат: ${selected.format.toLowerCase()}; нагрузка: ${selected.hours} часов`,
                  ...
                  ? "Завершение уже записано в демонстрационной истории"
                  : started.includes(selected.id)
                    ? "Занятие уже добавлено в ваш план — можно продолжить"
                    : "В демонстрационной истории это занятие ещё не завершено",
              ].map((f) => (
                <li key={f}>
