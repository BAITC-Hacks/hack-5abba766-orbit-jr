...
Используется серверный адаптер **OpenAI-совместимого Chat Completions API** со строгой JSON Schema. В [.env.example](.env.example) задана модель `gpt-6-luna`; для неё код устанавливает `reasoning_effort: low`, чтобы ранжирование укладывалось в бюджет ответа. Можно указать другую модель и полный URL совместимого API, но её поддержку Structured Outputs и качество ответов нужно проверять отдельно.
