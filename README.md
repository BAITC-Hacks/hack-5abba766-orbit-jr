...
## Проверка запуска из чистого клона — 23 сентября 2026

Код `d4b1670` заново клонирован с GitHub в пустой каталог. Запуск выполнен по
инструкции ниже: `.env.example` → `.env`, четыре исходных файла организаторов в
`data/source/`, затем `docker compose up --build -d`. Изменены только свободные
порты: `APP_PORT=3215`, `DB_PORT=54339`. Созданы новые контейнеры и отдельный пустой
volume PostgreSQL; старые `node_modules`, `.next`, `.env` и база не использовались.

**Результат:** оба контейнера `healthy`, `/api/health` вернул `status=ok`,
`dataset_initialized=true`, миграцию `003_learning_external_completion`.
В браузере проверены вход сотрудника, рекомендации, сохранение урока при
закрытии/повторном открытии, неверный тест без начисления, успешный тест и
сохранение прогресса **55% → 61%** после обновления страницы. HR увидел
**200 сотрудников и одно новое симулированное завершение**. Ошибок консоли
в этом проходе не обнаружено. Инструкция запуска воспроизвелась без правок кода.

Проверка выполнена без ключа AI (`ai_configured=false`, подбор по правилам).
Для демонстрации обязательного AI-слоя жюри нужно настроить провайдера по шагу 4;
эта проверка запуска не заменяет live AI-аудит и не подтверждает SLA под нагрузкой.
...
Главный вывод: сильная сторона — **проверяемый следующий шаг и диагностика ограничений каталога**. Преимущество LLM над собственными правилами пока не доказано; общий сценарий развития по навыкам уже существует у других платформ. Свежий прогон Luna с `low` дал **23/23** принятых AI-ответов и 100% совпадения top-1 с baseline. Предыдущий `xhigh` дал 22/23 с одним timeout; оба результата сохранены.

В [полном ТЗ Halyk](https://docs.google.com/document/d/18SlxWxz_Vj_eag_UfEQ6t6g59jCxGTgu5x4QoylDbqI/edit) веса отличаются: соответствие 25, техника 25, README/воспроизводимость 25, ценность 15, развитие/оригинальность 10. Приоритет рубрик неизвестен; исходные оценки относятся к картинке участника. [Матрица обязательных требований](docs/JURY_REVIEW.md#соответствие-полному-тз) отделяет реальные условия приёмки от гипотез бизнес-эффекта.

После объединения командных изменений до `e21be49` проверены **464 backend-теста, 79 клиентских, 61 PostgreSQL integration** (1 отдельный live-тест пропущен в обычном наборе), production-сборка и отдельный live-сценарий **3/3**. Браузерный проход сотрудник → два шага → HR, сохранность после рестарта и импорт авторских JSON/CSV проверены отдельно. Полный SLA UI ≤2 с / AI ≤10 с под нагрузкой не подтверждён. [Результаты и команды](docs/JURY_VERIFICATION.md).
...
- В каталоге видны допустимые роли, грейды и предварительные навыки. HR сохраняет режим просмотра без действий за сотрудника.
...
| Обучение по навыку | «Найти обучение» у разрыва открывает каталог с фильтром по навыку, возможным приростом, условиями участия и пояснением, если программ нет |
...
Используется серверный адаптер **OpenAI-совместимого Chat Completions API** со строгой JSON Schema. В [.env.example](.env.example) задана модель `gpt-6-luna`; для неё код устанавливает `reasoning_effort: low`. В дополнительном [аудите ML/AI](docs/ASTRA_ML_AUDIT.md) режим `xhigh` часто не укладывался в 8 секунд при выборе нескольких карточек; `low` выбран после сравнительных live-проверок. Можно указать другую модель и полный URL совместимого API, но её поддержку Structured Outputs и качество ответов нужно проверять отдельно. Старые отчёты относятся к указанным в них версиям промпта и проверок.
...
AI-слой обязателен по ТЗ. Резервный подбор обеспечивает доступность, но на защите
нужно отдельно показать принятый ответ `mode=ai`; репетиция с правилами не заменяет
эту проверку. Совпадение с сильным baseline не нарушает ТЗ, однако не доказывает
добавочную ценность AI. Однофакторное правило под видом AI недопустимо.

Общий бюджет ожидания провайдера — до 8 секунд, включая максимум одну повторную попытку при сетевом сбое или временной HTTP-ошибке. Повтор учитывает `Retry-After`, оставшееся время и отмену запроса. Ошибки доступа и невалидные ответы не повторяются. Число одновременных подборов ограничено; успешные AI-ответы кэшируются на 60 секунд с учётом входных данных и конфигурации. Изменение профиля требует актуального подбора. При длительной недоступности провайдера остаётся явно обозначенный резервный режим.
...
Подробности: [серверные компоненты, правила и API](docs/BACKEND.md), [устройство backend](backend/README.md).
...
Официальный набор и `.env` не входят в репозиторий. По ТЗ исходные синтетические данные нельзя выносить за пределы хакатона; публично распространять их нельзя. Наши live-регрессии используют собственные авторские записи. Без исходных файлов первый запуск завершится ошибкой загрузки; приложение не подставляет демонстрационные данные вместо них. После успешной инициализации обычный рестарт использует сохранённую БД.
...
Если порты `3000` или `54329` уже заняты, до запуска измените значения в `.env`,
например `APP_PORT=3215` и `DB_PORT=54339`. Оставьте `APP_ORIGIN` пустым для
автоматического origin. Тогда открывайте `http://localhost:3215` и
`http://localhost:3215/api/health`; остальные команды остаются прежними.
...
Дождитесь `healthy` у `app` и `db`: команда с `-d` может завершиться до готовности
приложения. Первый запуск создаёт схему и загружает данные. Строка
`.env not found. Continuing without it.` внутри контейнера штатная: Compose
передаёт настройки через переменные окружения, файл с ключами в образ не копируется.
...
Дополнительный [аудит и исправления 23 сентября](AUDIT-2026-09-23.md) описывает ограничения ожидания PostgreSQL, уточнения интерфейса, проверки объединения и открытый дефект skip-link. По срочному указанию завершить работу выполнены 5 из 15 запланированных агентских проходов; полный аудит всех сценариев не заявляется.

**Актуальное дополнение после аудита пяти судей:** [JURY_VERIFICATION.md](docs/JURY_VERIFICATION.md)
фиксирует проверки объединённой версии `673d2fa`, включая настоящий браузер,
перезапуск, импорт и свежий live AI. Ниже сохранён предыдущий этап для истории.
